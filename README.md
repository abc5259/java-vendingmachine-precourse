...
- [x] 자판기의 보유 금액을 입력받는다.
- [x] 보유 금액을 이용해 자판기의 동전을 무작위로 생성한다.
- [x] 자판기가 보유한 동전을 출력한다.
- [x] 상품명과 가격, 수량을 입력받는다.
    - [x] 예외1. 상품 가격은 100원 미만인 경우
    - [x] 예외2. 상품 가격이 10원으로 나누어떨어지지 않는 경우
    - [x] 예외3. 상품 수량이 숫자가 아닌경우
    - [ ] 예외4. 중복된 상품이 있는 경우
- [x] 투입 금액으로 상품을 구매한다.
- [x] 남은 금액이 상품의 최소 가격보다 낮은 경우 잔돈을 반환
- [x] 모든 상품이 소진된 경우 바로 잔돈을 반환
- [x] 잔돈 반환시 현재 보유한 최소 개수의 동전으로 반환
- [x] 잔돈을 모두 반환할 수 없는 경우 잔돈으로 반환할 수 있는 금액만 반환
- [x] 잔돈 출력후 종료
